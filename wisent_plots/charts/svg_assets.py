...
def _asset_path(*parts):
    return os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", *parts
    )
...
def _create_cartesian_patterns(svg, style: int):
    ...
    if style == 2:
        _load_pattern_from_file(
            defs, "pattern-noise", os.path.join(assets_dir, "large", "noise_rectangle_large.svg")
        )

    # Style 3: crossing lines for middle column
    elif style == 3:
        _load_pattern_from_file(
            defs, "pattern-crossing", os.path.join(assets_dir, "pattern_crossing_lines.svg")
        )
    ...
    # Style 4: noise + dither crosses
        ...
        _load_pattern_from_file(
            defs, "pattern-dither", os.path.join(assets_dir, "large", "dither_cross_large.svg")
        )
...
def _load_pattern_from_file(defs, pattern_id: str, svg_file: str):
    ...
    pattern_elem = ET.SubElement(
        defs,
        "pattern",
        {"id": pattern_id, "patternUnits": "userSpaceOnUse", "width": width, "height": height},
    )
