...
from wisent_plots.charts.svg_components import render_bubble_legend, render_title
...
class SVGRadarBubbleChart:
    ...
    def create_chart(
        ...
        # Render title
        render_title(svg, title, self.colors, self.padding_x, self.padding_y)
        ...
        if category_labels and categories:
            ...
            filtered_labels = [
                category_labels[i] for i in unique_categories if i < len(category_labels)
            ]
            render_bubble_legend(
                svg, filtered_labels, self.colors, self.padding_x, self.padding_y, unique_categories
            )
        ...
        # Render radar grid
        self._render_radar_grid(
            svg, chart_center_x, chart_center_y, max_radius, num_rings, num_axes
        )
    ...
    def _render_radar_grid(
        self, svg, cx: float, cy: float, max_r: float, num_rings: int, num_axes: int
    ):
