...
for style in [1, 2, 3, 4]:
    chart = AreaChart(style=style, edge=True)
    ...
    filename = f'examples/area_chart_style{style}_final.svg'
    with open(filename, 'w') as f:
        f.write('<?xml version="1.0" encoding="UTF-8"?>\n')
        f.write(svg_output)
    
    print(f"✓ Style {style} saved to {filename}")
